...
std::vector<
    TransportNetwork::GraphEdge
>::const_iterator TransportNetwork::GraphNode::FindEdgeForRoute(
    const std::shared_ptr<const RouteInternal>& route
) const
{
    return std::find_if(
        edges.begin(),
...
bool TransportNetwork::FromJson(nlohmann::json&& src)
{
    if (src.empty())
    {
        throw std::runtime_error("runtime_error::TransportNetwork::FromJson(nlohman::json&&)::input src empty");
    }
    ...
    for (auto&& lineJson: src.at("lines")) {
        Line line {
            ...
            std::vector<Route>{}, // We will add the routes shortly.
        };
